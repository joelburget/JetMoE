...
class top_k_gating(nn.Module):
    ...
    def forward(self, x):
        ...
        # take the top k + 1 experts
        top_k1_logits, top_k1_indices = logits.topk(self.top_k + 1, dim=1)
        # now remove the top expert
        top_k_logits = top_k1_logits[:, 1:]
        top_k_indices = top_k1_indices[:, 1:]
